...
CHIP_ERROR BLEManagerImpl::StartAdvertising(void)
{
    ...
    uint32_t minAdvInterval = (NumConnections() == 0
#if 0
                      // Temporarily disabled until ConfigurationMgr is available
...
#if 0 // Disabled temporarily until ConfigurationMgr is available
                                    ...
                                    PacketBuffer * pBuf)
{
    ...
    params->uuid = nullptr;
    params->attr = &CHIPoBLE_Service.attrs[3];
    params->func = BLEManagerImpl::HandleTXIndicated;
...
ssize_t BLEManagerImpl::HandleRXWrite(struct bt_conn * conId, const struct bt_gatt_attr * attr, const void * buf, u16_t len,
                                      u16_t offset, u8_t flags)
{
    ChipDeviceEvent event;
    PacketBuffer * packetBuf = PacketBuffer::New();
...
ssize_t BLEManagerImpl::HandleTXCCCWrite(struct bt_conn * conId, const struct bt_gatt_attr * attr, u16_t value)
{
    ChipDeviceEvent event;
...
void BLEManagerImpl::HandleTXIndicated(struct bt_conn * conId, const struct bt_gatt_attr * attr, u8_t err)
{
    ChipDeviceEvent event;
...
void BLEManagerImpl::HandleConnect(struct bt_conn * conId, u8_t err)
{
    ChipDeviceEvent event;
...
void BLEManagerImpl::HandleDisconnect(struct bt_conn * conId, u8_t reason)
{
    ChipDeviceEvent event;
